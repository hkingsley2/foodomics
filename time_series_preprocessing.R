...
#Load the daily data
daily_intakes<-read.csv(file='015886550_intake.csv', header=TRUE, sep=",", na.strings=c("","NA"))
...
data_type<-read.csv(file='015886550_intake.csv', header=TRUE, sep=",", na.strings=c("","NA"))
...
day_type<-read.csv(file='015886550_intake.csv', header=TRUE, sep=",", na.strings=c("","NA"))
...
diet_changes<-read.csv(file='thsc_ndt.csv', header=TRUE, sep=",") #NEED TO ADD MRNUMBER TO THIS FILE
diet_changes<-diet_changes[,c(1,4,5,6,8,9,10,12,13,14,15)] ###WILL NEED TO CHANGE THESE COLUMN IDS
