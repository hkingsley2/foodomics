#Import required database files
    ...
    ####GET COMPILED DB
              ...
              compiledNFD<- read.csv(file = "formatting_condensed_db_export_2.txt", header = TRUE, sep="\t", na.strings=c("NA","NaN", " ", "N/A", ""),stringsAsFactors=FALSE)
              ...
              compiledNFD[,c(14:104,123:124,141:250)]=as.numeric(apply( compiledNFD[,c(14:104,123:124,141:250)], 2, function(x) x *  compiledNFD$NFL_Factor))
              # NFL[NFL==""] <- NA
              #Now it's all per 100 grams
              
    ####GET REFERENCE BASE PRODUCT PROFILE DATABASE
              
              referenceBASEP<-readRDS(file = "reference_BASEP_based_on_12162015_compiledDB.rds")

              
    For each food in NFD database (compiled< match food in reference dataabse, and merge the two)
    
    
    ####GRAND MERGED UNCALCULATED FOODOMICS UNREPLICATED
    
    unCALC_unREPL_foodomicsDB<-merge(compiledNFD, referenceBASEP, by="New.NDID.Number")  #this is dropping things that don't have matches??????
    #rename for simplicity
    UCURdb<-unCALC_unREPL_foodomicsDB
    
####NOW FOR CALCULATING FOODOMICS
   ...
   #SUMMARY STATS SO FAR - none of these are very good
   
               #CHO_stats <- lapply( UCURdb[,c(412:423)] , function(x) rbind( mean = mean(!is.na(x)) ,
                                                        sd = sd(!is.na(x)) ,
              ...
              # CHO_stats<- data.frame( CHO_stats )
              
               # subset(UCURdb, FNA_SUGAR>100)
                
                #na_count <-sapply(UCURdb[,c(412:423)], function(y) sum(length(which(!is.na(y)))))
                #CHO_na_count <- data.frame(na_count)
    ################
    #PRO CONVERSIONS
    ################
   ...
   UCURdb$FNA_FATRN<-ifelse(UCURdb$Total_Fat_per_serving_g=="0",0,ifelse(!is.na(UCURdb$Trans_Fat_per_serving_g),UCURdb$Trans_Fat_per_serving_g,UCURdb$FAT_conv*UCURdb$`X605`))
    ...
    #not sure if this is correct placement of the code
    #UCURdb$FAMSratio<-ifelse(UCURdb$FAMS>=0,UCURdb$FAMS=0,UCURdb$Mono_conv)
    #UCURdb$FAPUratio<-ifelse(UCURdb$FAPU>=0,UCURdb$FAPU=0,UCURdb$Poly_conv)
    #UCURdb$FASATratio<-ifelse(UCURdb$FASAT>=0,UCURdb$FASAT=0,UCURdb$Sat_conv)
    #UCURdb$FATRNratio<-ifelse(UCURdb$FATRNratio>=0,UCURdb$FATRNratio,UCURdb$Trans_conv)
